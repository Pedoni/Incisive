...
''';
  ...
  void _sendMessage() {
    ...
    _controller.clear();
    setState(() => _messages.insert(0, ChatMessage(true, text)));
    _scrollToTop();

    final payload = _buildConversationPayload(text);
    context.read<ChatBloc>().sendMessage(payload);
  }
  ...
  Widget _chatBubble({required bool isUser, required String text}) {
    return Align(
  ...
  Widget build(BuildContext context) {
    return BlocConsumer<ChatBloc, BaseState>(
      listener: (context, state) {
        if (state is Success) {
          final reply = state.data as String;
          setState(() => _messages.insert(0, ChatMessage(false, reply)));
          _scrollToTop();
        }
        if (state is Error) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text("C'è stato un problema. Riprova tra poco."),
            ),
          );
        }
      },
      builder: (context, state) {
        final isTyping = state is Loading;
                    ...
                    child: ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return Padding(
                          ...
                          child: _chatBubble(isUser: msg.isUser, text: msg.text),
                        );
                      },
                    ),
                  ),
                  _buildInputBar(isTyping: isTyping),
                ],
              ),
            ),
          ),
  ...
  Widget _buildInputBar({required bool isTyping}) {
    return Padding(
        ...
        children: [
          Expanded(
            child: Container(
              ...
              decoration: BoxDecoration(
                color: const Color.fromARGB(255, 241, 218, 192),
